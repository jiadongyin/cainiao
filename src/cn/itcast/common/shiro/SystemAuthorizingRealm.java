...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		String role = userDao.fingRole(currentUser.getUser_id());
        simpleAuthorInfo.addRole(role);  
        //按照userid从数据库中获取权限
        String permission = userDao.findPermission(currentUser.getUser_id());
        simpleAuthorInfo.addStringPermission(permission); 
        logger.debug("角色信息: {}", role.toString());
        logger.debug("权限信息: {}", permission.toString());
